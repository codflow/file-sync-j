...
public class SftpEndpointPanelContainer extends AbstractEndpointPanelContainer implements EndpointPanelContainer {
	...
	public ClientEndpointBO getEndpointBO() {
		...
		return ClientEndpointBO.builder().id(generateEndpointId()).name(name).type(ClientTypeEnum.SFTP).authenticationBO(authenticationBO).rootPath(path).build();
	}
